...
<script type="text/javascript">
   ...
   //    var filter_user_type = "";
   //    var base_url = "<?php echo base_url(); ?>";
   //    var data_table = $('#step1details_datatable').DataTable({
   //       "pageLength": 10,
   //       "serverSide": true,
   //       "order": [
   //          [0, "asc"]
   //       ],
   //       "ajax": {
   //          url: base_url + 'mydddapplicationform/get_step1details',
   //          type: 'POST',
   //       },
   //    });
   // });
   ...
   //          $('#step1_id').val(data_id);
   //          $('[name="step1_id"]').val(result[0].step1_id);
   ...
   //    var base_url = "<?php echo base_url(); ?>mydddapplicationform/get_step2_payment_details/";
   //    $.ajax({
   //       type: "GET",
   //       url: base_url + data_id,
   //       success: function(data) {
   ...
   // $(document).on('click', '.step2', function(e) { //step2_details button - view step2_details details by user_id on modal
   //    e.preventDefault();
   //    var data_id = $(this).attr('data-id');
   //    let new_array = [];
   //    let checker = '';
   ...
   //          $('[name="agency_quote"]').val(result[0].agency_quote);
   //          $('[name="website_invoice"]').val(result[0].website_invoice);
   //          $('[name="agency_invoice"]').val(result[0]._invoice);
   ...
   $(document).on('click', '.step3', function(e) { //step3_details button - view step3_details details by user_id on modal
      e.preventDefault();
      var data_id = $(this).attr('data-id');
      let new_array = [];
      let checker = '';
   ...
   $(document).on('click', '.step4', function(e) { //step2_details button - view step2_details details by user_id on modal
      e.preventDefault();
      var data_id = $(this).attr('data-id');
      let new_array = [];
      let checker = '';

      var base_url = "<?php echo base_url(); ?>mydddapplicationform/get_step4_details/";
      $.ajax({
         type: "GET",
         url: base_url + data_id,
         success: function(data) {
            let result = JSON.parse(data);
            $('[name="selected_prototype"]').val(result[0].selected_prototype);
            $('[name="fingerprint_card_details"]').val(result[0].fingerprint_card);
            $('[name="criminal_disclosure_details"]').val(result[0].criminal_disclosure);
            $('[name="reference1_details"]').val(result[0].reference1);
            $('[name="reference2_details"]').val(result[0].reference2);
            $('[name="reference3_details"]').val(result[0].reference3);
            $('#step4').modal('show');
         },
         error: function(data) {
            alert(data);
