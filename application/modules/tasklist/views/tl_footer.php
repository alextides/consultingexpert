...
$(document).ready(function(e){
  ...
  var data_table = $('#tasklist_datatable').DataTable({
     "pageLength" : 10,
     "serverSide": true,
     "order": [[0, "asc" ]],
     "ajax":{
              url :  base_url+'tasklist/view_tasks',
              type : 'POST',
            },
      });
  ...
  $(document).on('click','.edit_user',function(e){
     ...
     var base_url = "<?php echo base_url();?>tasklist/edit_user/";
     $.ajax({
         type : "GET",
...
$(document).ready(function(){
 ...
 $('#username').on('blur', function(){
  ...
  $.ajax({
    url: '<?php echo base_url();?>tasklist/verify_username',
    type: 'post',
 ...
 $('#email').on('blur', function(){
  var email = $('#email').val();
  $.ajax({
    url: '<?php echo base_url();?>tasklist/verify_email',
    type: 'post',
    data: {'email' : email},
    success: function(response){
      if (response == 'taken' ) {
 ...
 $('#add_technician_form').on('submit', function(e){
  ...
  if (username_state == false || email_state == false) {
    $('.error_msg').text('Replace input on highlighted field/s.');
     ...
     $.ajax({
        url: '<?php echo base_url();?>tasklist/add_user',
        type: 'post',
        data: form_data,
        success: function(response){
